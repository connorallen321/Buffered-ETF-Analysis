...
"""
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Return only structured ETF data."},
...
def draw_chart(cap, current, buffer_start, buffer_end, floor=0.0):
    ...
    segments.append((0.0, unbuffered_top, 'darkred', f'Unbuffered Loss ({(unbuffered_top / current) * 100:.1f}%)'))
...
if st.button("Generate Inputs"):
    ...
    if not current:
        ...
        if None in [cap, buffer_start, buffer_end]:
            st.error("❌ GPT failed to return usable inputs.")
        else:
